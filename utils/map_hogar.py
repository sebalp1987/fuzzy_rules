...
def hogar_mapping(hogar_df: pd.DataFrame,  range_acceptance=90, scorer=default_scorer, separate_odd=True,
    ...
    for index, row in tqdm.tqdm(hogar_df.iterrows(), total=hogar_df.shape[0]):
        ...
        hogar_tipo_via = row['hogar_tipo_via']
        ...
        list_coincidences.append([hogar_tipo_via, hogar_nombre_via, hogar_numero_via, hogar_cp, coincidence_name, score,
                                  total_code])

    cleaned_df = pd.DataFrame(list_coincidences, columns=['hogar_tipo_via', 'hogar_nombre_via', 'hogar_numero_via',
